...
**Exo baseline: both stats are 0.** Consistent with Exo's permanent zero-defense identity — neither stat grows through normal suit progression. Any Hazard Resistance/Defense Exo ever has would have to come from modules (not yet designed — deferred to a future session, see Open Questions).
...
- **Exo — both stats 0, module-only.** Not yet designed which modules (if any) grant Hazard Resistance/Defense, or how much. Deferred.
...
- Exo's hazard modules — which ones exist, what they grant, not designed.
