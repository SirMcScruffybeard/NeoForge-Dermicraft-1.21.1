...
**Placement:** Introduced at the **Stage 2/3 boundary** — every ingredient (Dragon's Milk, Molten Quartz, Blaze Essence) is a Stage 2 fluid, but the result and everything it produces (Living Glowstone, Living Netherite, Living Metals) is Stage 3. Not pinned to an exact Tier yet; likely **Tier 3 or Tier 4** territory for any machine that needs to *handle* a Living fluid at scale, separate from the Gestator itself which only needs to *produce* it.
...
**Density:** Set **negative**, using NeoForge's FluidType behavior where negative density makes a fluid flow *upward* instead of pooling downward. A deliberate physical "wrongness" lever, parallel in spirit to Primitive Catalyst's inverted motion scale, but using a different property so it isn't a repeat of the same trick. (Was originally going to be the mod's first fluid to use this property, but **Blaze Essence shipped first** — see `dermicraft-crafting-notes.md` — since it's a Stage 2 fluid built well before Living Glowstone's Stage 3. Living Glowstone remains the second, not the first.)
