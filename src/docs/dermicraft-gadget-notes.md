...
- **Field-swappable via the Scrench**, generalizing Sunder's existing chain-swap flow (`dermicraft-gadget-notes.md` → Scrench, and the Workbench's own Swap page as the safer/costless station counterpart) to Modules broadly, not just Sunder's chain/Drill Hammer's head.
...
**Suggested starting point (not yet acted on):** categorize the existing upgrade catalog (Eater's material categories, Sunder/Drill Hammer's abilities, M.A.W.S.'s features, etc.) by what they actually ARE, independent of how they're currently acquired (points vs. parts) — build the taxonomy first, then decide per-category which acquisition mechanism (points, Module, or both) fits, rather than starting from "convert everything to Modules" or "keep everything as points."
...
- Exact taxonomy for every OTHER gadget's catalog (Sunder/Drill Hammer's abilities, M.A.W.S.'s features) — Eater is the only one worked through so far.
