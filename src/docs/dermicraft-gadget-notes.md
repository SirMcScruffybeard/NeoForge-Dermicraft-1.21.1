...
- **Sunder specifically** also layers in a **tier ladder** (Growth Chamber) on top of the point-spend + physical-part combo above — capability leaps only (new verbs/thresholds), not smooth scaling, per the mod's godlike-but-earned endgame philosophy. See Sunder's own entry for its full four-axis breakdown (chain material / tier / points / fuel grade).
...
**Open questions:** Recipes/crafting chain — not yet touched.
...
- **Trees:** no separate time cap — the fuel/hunger budget itself is the limiter (see Tree Felling below). Bigger trees cost more resource, not more difficulty, since tree size stopped being a problem once the instant one-hit-fell was moved off base tier.
...
- Higher tier restores **instant one-hit tree felling** as a capability leap layered on top of this base behavior — see Tier Ladder below.

**Tree Felling (2026-08-01):** Standard tree-capitator-style contiguous log detection (flood-fill from the hit log, connected logs only), adapted to the mod's own style — **only affects logs**, leaves are untouched.
...
- **Safety cap on the flood-fill's scanned-block count** — a performance/technical guard, not a gameplay number. Without one, touching canopies between adjacent trees could chain the fill across an entire forest in one swing, which is both a bad felling result and a tick-cost risk. Once the cap is hit, whatever's been found so far is treated as "the whole tree."
- **Total tree health = per-log cost × detected log count.** Pulses deal the same fixed per-pulse damage the combat side uses — no separate tree-specific damage stat. Tree size therefore raises resource cost, not time-to-kill difficulty (no time cap on trees, per the core mechanic above).
- When the health pool is exhausted, **the whole tree comes down at once** (not incremental per-log removal during the dig-in).
- **Open/unresolved:** partial-completion behavior if fuel+hunger run dry before a large tree's health pool is exhausted (does progress persist for a later resume, or reset).
...
**Stat layering model (2026-08-02, new) — five inputs (base, tier, chain, fuel, points) combined via vanilla's own `AttributeModifier.Operation` tiers rather than a bespoke formula, chosen specifically so the combination behavior is a proven, off-the-shelf system rather than something hand-maintained:**
...
- **`ADD_MULTIPLIED_BASE`** (percentage-of-base bonuses that sum with each other, applied once as a group — deliberately **summed, not compounding**, specifically to avoid runaway/exponential growth as more tiers, chain materials, and fuel grades get added over the mod's lifetime, and to keep tier reading as a controlled capability leap rather than a value that multiplies unpredictably against whatever chain/fuel happen to be equipped, mirrors why vanilla itself keeps enchantments/most potion bonuses in this same summed tier): tier's stat contribution, chain's damage multiplier (unified across standard-hit and dig-in for now, split later only if needed), fuel grade.
- **`ADD_MULTIPLIED_TOTAL`** (a final multiply, applied last, the **one** deliberately compounding layer): points' multiplicative component — self-limiting via the existing escalating point-cost curve (see the shared Gadget upgrade points section above), and guaranteed to apply last as a structural property of vanilla's own calculation order rather than something to maintain by hand.
- Mirrors the suit's already-confirmed layering language ("base → tier adjustments → the result becomes the new base → point-spend calculated after that") — tier's output becomes the next layer's starting point, same shape, just with chain and fuel now inserted into the pipeline too.
...
- **Tier ladder (Growth Chamber):** capability leaps only, no smooth scaling on its own — base tier is the rev-and-dig-in mechanic existing at all; a higher tier unlocks **instant one-hit tree felling** as a new threshold. Also contributes to `ADD_MULTIPLIED_BASE` in the stat layering model above.
...
- **Texture layout (2026-08-01):** one **base texture** covering the body (grip/guard/arm brace/engine block — unaffected by material or state) plus a **separate chain render layer** (mirrors `SippingGlowLayer`'s structure, see [[project_geckolib_pipeline]]) holding the two grayscale chain textures (still/running) described above. The chain is pulled off the base texture's UV canvas entirely rather than sharing it, since GeckoLib's base render pass pairs one model with one texture per call (`getTextureResource(T)`) — swapping just the chain's texture independently of the body requires it to be an additional layer, the same mechanism already proven for Drinker's gauge/screen-state textures and Sipping's glow layer.
...
**Open questions:** Per-log tree health cost and flood-fill scan cap (both intentionally left for after the upgrade model, since the point axes should make them easier to tune). Rev-up time, pulse damage, pulse cooldown length, mob time cap length, fuel drain rate per pulse. Exact standard-hit Bleed/decapitation chance within the ~20-25% range. Broken-chain standard-hit damage reduction amount. Whether the flood-fill's diagonal/gap-hop detection has any material/tier gating or is available from base tier. Whether dig-in gets additional chain vibration on top of the idle animation's existing movement, or reuses it as-is (see Model & animation above — tentative, not committed). Exact tick timing beyond the two confirmed 10-tick delays (rev-up animation length, reverse/rev-down animation length). Chain durability's actual data-component shape and the vertical hotbar-bar rendering hook (both designed, not yet built). Which specific chain materials (if any) get a special-case status effect, and what those effects are. Exact per-tier/chain/fuel percentages feeding the `ADD_MULTIPLIED_BASE` sum.
...
**Open questions:** Scrench's own crafting recipe/cost. Exact movement-penalty magnitude and duration. Whether Drill Hammer's head-swap needs any Scrench-flow differences beyond swapping "chain" for "head" (not yet checked against Drill Hammer's own entry below).
