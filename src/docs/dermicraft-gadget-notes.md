...
**Status:** Concept stage — renamed from Drill Hammer 2026-08-11 alongside a real mechanical rework (not just a rename); most core mechanics re-decided, exact numbers/the charge special's actual effect still open.
...
**Controls:**
- **Left-click:** a standard hit — flat 3×3 face mining pattern, 1 block deep uniformly (no 2-deep center — that was a drill-specific shape tied to the old head design, retired with it). Matches the common modded-hammer convention rather than anything bespoke.
- **Right-click (hold to charge, release on a valid target to fire):** a charged special, meant to hit noticeably harder than the standard 3×3 swing. Distinct input shape from Sunder's rev-then-auto-lock (hold-and-sustain) — this is closer to a drawn-bow pattern (charge, then release-to-fire), and should NOT reuse Sunder's state-machine shape wholesale when implemented. **Still open:** what the special actually does beyond "bigger than the standard swing," charge cap/max-charge behavior, and what happens on release with no valid target (fizzle vs. wait).

**Targeting logic** (governs whether the AoE hits blocks or mobs, applies to the left-click standard hit):
...
- Heavy bonus durability damage against armor, applied **uniformly across all armor types** — the old chainmail-resists-it exception is dropped (2026-08-11): it was reasoned from drill/piercing logic (mail stops punctures), which doesn't hold for a blunt piston impact. Real chain mail is if anything *especially* weak to blunt trauma (maces/warhammers historically countered it), so a hammer-flavored exception in the other direction wasn't justified either — simplest resolution was just removing the exception rather than flipping it.

**Heads (material-based, swappable) — reworked 2026-08-11:**
- Mount/dismount mechanic mirrors Sunder's chain system exactly (Scrench field-swap + Workbench path, Iron as the baseline material, no head installed = damage-penalized fallback with **no special** available at all — same shape as Sunder's no-chain state).
- **Each head is a distinct special, not a linear power curve** — deliberate departure from the old "vanilla material curve" framing (Iron < Diamond < Netherite as a straight damage ladder). Mining strength still maps onto the **vanilla pickaxe tier system as a hard gate** (same as vanilla pickaxes — a sub-tier head simply cannot break a block above its tier, not a slower/weaker attempt at it), but each head's *combat*/special identity is its own thing per material, not derived from that tier.
- **Module system as the tier-bypass path:** a Module can let a lower-tier head break a block above its vanilla-gated tier (e.g. an Obsidian-bypass Module on an Iron head) — ties head-swapping into the Module direction already noted under Gadget upgrade points above, rather than inventing a separate bypass mechanism.
- Vanilla materials (Iron, Diamond, Netherite, etc.) are the confirmed starting point; non-vanilla/exotic head materials and their specific special effects not yet decided.

**Fuel (Slurry grade) — 2026-08-11, decided:**
...
**Open questions:** What the charge special actually does mechanically (beyond "bigger than 3×3"). Charge cap/max-charge behavior and release-with-no-target handling. Exact bonus-damage/status-effect values vs. skeletons. Non-vanilla head materials and their per-head special effects. Exact per-grade fuel-use-rate/speed numbers for Shatter specifically (the Main Line curve itself has values, but Shatter's own consumption-per-charge/per-swing hasn't been set).
