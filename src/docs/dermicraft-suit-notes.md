...
- **Feeder Bladder Module** *(renamed from "Feeder Tank"/"Feeder Bladder" to match naming convention)* — same Bladder-family origin as Fuel Bladder, but serves the **player's hunger** instead of the suit's fuel. Feeds the player **Slurry or Protein Blend**, restoring hunger; **may carry additional effects depending on which fluid is fed** (mechanic and specifics deferred — to be designed later). **Both an item and a module**, same dual-mode pattern as the rest of the Bladder family: usable standalone (manually consumed/activated by the player) or docked into the universal Module Frame and slotted into a suit for automatic, passive feeding. **Hunger-restoration cost uses the mod-wide Fluid-to-Hunger Conversion rate** (50 mB Crude per hunger point, scaling down with Heal modifier for better grades — see `dermicraft-slurry-notes.md`).
