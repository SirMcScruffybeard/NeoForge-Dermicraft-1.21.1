...
- **Naming/shape note:** a Lens doesn't fit the "[Equipment name] Module" naming convention or the Equipment-origin rule the way most modules do — it has no standalone form at all, only two host-dependent ones (Frame-inserted or suit-slotted). This is closer to Sunder's chain/Shatter's head "swappable identity part" pattern (see `dermicraft-progression-notes.md`'s taxonomy) than a normal Module, though it also directly occupies a suit Module slot the way a Module does — worth a proper taxonomy pass later rather than forcing a label now.
