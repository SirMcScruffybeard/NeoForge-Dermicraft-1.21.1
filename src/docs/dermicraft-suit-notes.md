...
### Head — Goggle Frame & Lenses (simplified 2026-08-29, replaces the old four-item design)

**Old design retired:** Bio/Night/Ore/Fluid Vision were previously four separate items, each *both* a standalone vanilla-head-slot item *and* its own suit Module, plus a late-game (Stage 3/4) **Goggle Frame** that could hold up to four of them as swappable inserts. That's replaced entirely by a single system, available from **early game**, not gated to Stage 3/4:

- **Lenses** — Bio Vision, Night Vision, Ore Vision, Fluid Vision (and presumably more later — "and so on") are now **Lenses**, not standalone dual-mode items. A bare Lens has no independent vanilla-head-slot use of its own — it only works inserted into one of the two hosts below.
- **Goggle Frame** — the actual **equippable item**, worn in the vanilla head/helmet slot (same real-defense tradeoff the old standalone goggles had). Holds Lenses: starts with **one slot**, upgradeable up to **four** via "upgrade the base item, then re-slot" (same rule as the Rack) — unchanged from the old Goggle Frame's own upgrade path, just no longer gated behind Stage 3/4.
- **Suit integration — the Lens itself slots in, not the Frame.** When used with a suit, it's the individual **Lens** that occupies the suit's own **Head Module slot** directly — the Frame plays no role in suit integration at all. This means a suit's Head-slot vision capability is one Lens per Head slot the suit has (Exo/Recon's own slot counts, not the Frame's 1–4 upgrade track), a genuinely different resource from the handheld Frame's own capacity.
- **Taxonomy (confirmed 2026-08-29): a Lens is a Dual-Host Part, not a Module** — its own fifth shape in `dermicraft-progression-notes.md`'s taxonomy, distinct from both normal Modules and Sunder's chain/Shatter's head "swappable identity part" pattern, though close kin to the latter (both are swappable identity parts, not Points/Tier ladder/Modules). What separates a Lens from Sunder/Shatter's own pattern: it has **no standalone form and no upgrade tier of its own** (the Goggle Frame's 1→4 slot count is the thing that upgrades, not the Lens), and it seats in **two** different hosts (Goggle Frame or a suit's Head Module slot) rather than one. Doesn't get its own "[Equipment name] Module" name for this reason — it's just "[Vision type] Lens."
...
- **Bat Wings Module** *(replaces Squid o' War, 2026-08-29 — same mechanic and slot, new equipment identity)* — full flight capability. Can also be mounted in Legs (same effect, alternate slot). **Equipment origin: Bat Wings** — a living membrane-wing graft inspired by real bat wing biology (the **patagium**, a skin membrane stretched across elongated finger-bones, powering true flapping flight) — same "inspired by, not literally sourced from" convention already used for Spider Gloves/Gecko Cleats, deliberately grounded body-horror flight imagery rather than the retired Squid o' War's gas-sac-and-jet-propulsion framing (too silly/Lovecraftian on reflection). **Worn in the vanilla Chest/chestplate slot** (not inventory-carry) — given how powerful full flight is, this is the most significant armor-slot tradeoff of any equipment origin so far (Chest being arguably the highest-value defense slot), appropriately scaled to the ability's outsized power.
  - **Fuel model (unchanged from Squid o' War):** fuel is only consumed for **active movement** — hovering in place is **free**, but causes a **slow altitude decay** (can't sustain hover forever for free, without an outright fuel drain either). Creates a real choice between drifting/planning for free vs. spending fuel to actually travel.
  - **Sprint + jump** triggers an accelerated climb — a hard wing-beat downstroke for fast vertical ascent (reflavored from the old jet-propulsion framing), not just "hold up to go up."
  - **Upgrades, two axes (unchanged):** movement speed, and reduced hover-drop rate.
  - **Max tier reward (unchanged):** hover-drop is fully eliminated at max investment — **perfect free hover**, a genuine "earned it" payoff rather than something available from the start, consistent with the mod's broader philosophy of real hassle building toward real reward.
...
- **Feeder Module (new, confirmed 2026-08-29 — replaces both Feeder Bladder and Feeder Craw as modules).** Passively manages player hunger by pulling directly from the player's **general inventory**, not from one dedicated Bladder/Craw item docked in the slot — a real mechanism change, not just a rename/merge of the two old entries.
  ...
  - **Configurable via a screen (confirmed 2026-08-29), most likely the Workbench.** Rather than an in-field toggle, restricting which inventory items/sources the module may draw from is set through a dedicated configuration screen — Workbench is the current best guess for where it lives (not fully locked), not the Dock, since this is a settings/filter concern rather than a Frame-slotting one.
  - **No standalone equipment origin (confirmed 2026-08-29) — Feeder Module is crafted directly as a module-only item**, never independently usable/carriable the way Bio Vision Goggles or Bat Wings are. It still physically occupies a Chest/Arm slot and docks via the Dock's Mod tab like any Module — it just has no dual-mode "standalone equipment" half. See the Equipment-origin rule below: this is now the **expected default** for future modules, not a one-off exemption.
...
- **Glider Module** *(renamed from "Glide" to match naming convention)* — a third flight-family option. Must be held in a hand to work (distinct from Bat Wings Module's Chest-slot requirement). Early on, cannot be combined with the other Flight modules (Chest/Legs).
...
- **Bat Wings Module** — see Chest entry; same module, alternate mount point.
...
- *(Landing Brace formerly listed here — resolved and moved to Exo's own Exclusive module section; see "The Exo-Skeleton (Base Suit)" above.)*
- **Grappler Anchor Module** *(renamed from "Grappling Brace"/"Grappler Anchor" to match naming convention)* — universal (all three suits). A Grappler-augment module (see the Grappler entry in `dermicraft-gadget-notes.md`), now with **its own standalone equipment origin** (resolving the earlier ambiguity of riding entirely on the Grappler's own equipment). **Design principle:** the module grants the **suit** the capability to use the Grappler this way — it isn't the Grappler item itself being empowered — so it works identically whether the Grappler is docked (Grappler Module, slotability TBD — see below) or simply carried on the hotbar, as long as the suit + this module are worn.
  - **Equipment origin — inventory-carry, not a vanilla armor slot.** Works as long as the physical Grappler Anchor item is **carried anywhere in the player's inventory** — same model as Auto Shield ("requires the player to be carrying an actual Shield item in inventory," no special slot consumed). **Explicitly not worn in the vanilla leg-armor slot** — that was considered and rejected: occupying that slot would cost the player their entire leg armor defense when not using a suit, a disproportionate tax compared to every other Gadget in the mod (all hotbar/handheld, none compete with armor). The "leg-mounted brace" framing survives purely as **cosmetic/lore flavor**, not a mechanical slot requirement.
  ...
  - **Effect 1 — hold a mob:** crouch **before firing** at a mob to **brace and hold it in place** (immobilized for as long as the brace is active; neither party is pulled — this is a hold, not a pull). Same **range** as the Grappler's normal fire range. **Fuel drain scales with the held mob's mass** — heavier mob costs more per tick to restrain. **No separate duration cap** — limited purely by available fuel, same as the suit's other continuous-drain modules (Self-Healing, Player Regen, Auto-Feeder). Pulling a *heavier* mob to the player (rather than just holding it) remains a possible future **companion module**, not part of Grappler Anchor.
  - **Effect 2 — PvP:** enables grappling **other players** and pulling them in **harmlessly** (no crouch needed for player targets) — the confirmed mechanism for PvP grappling, which the base Grappler otherwise disallows.
  - **Effect 3 — anchor self on a block:** crouch **before firing** at a **block** (rather than a mob) braces the *player* in place instead of pulling them toward the anchor — the player stays stationary until they stand up (release crouch), at which point normal movement resumes. **No added fuel cost** — unlike the mob-hold (Effect 1), self-anchoring is free beyond the normal shot cost, and its duration is bound directly to crouch state rather than fuel. Completes the module's core identity: crouch-before-fire always converts the Grappler's pull into a **brace/restrain** effect, whether that's restraining a mob (Effect 1) or restraining the player's own position (this effect) — only the PvP pull (Effect 2) doesn't need crouch. This overrides the base Grappler's pull-to-block behavior (and interactions like Momentum Retention/Controlled Drop) specifically under the crouch-before-fire condition; normal (non-crouch) block firing is unaffected.
...
**Equipment-origin rule — reversed (2026-08-29): standalone equipment is now the exception, not the default.** The original rule (kept below for history) required every module to originate from a real, independently-usable piece of equipment. Going forward, the **default is the opposite**: most modules are simply module-only items (or, for the six suit-scale-kinetic-event cases below, no item at all) with no separate standalone/handheld existence — **Feeder Module is the first confirmed instance of the new default**, not a special case (see its own entry under Chest, above). A standalone-equipment origin is still fine when it genuinely fits (Bio Vision Goggles, Bat Wings, the muscle grafts, etc. all keep theirs, grandfathered in), but it's no longer something every future module needs designed for it — the "large standing project" that used to require retrofitting one onto every existing module is **retired**, not just deprioritized.

**Old rule, kept for history (superseded above, 2026-08-29):** each module was required to originate as a **real, standalone piece of equipment** the player can use on its own (a Gadget), which then docks into a **universal Module Frame** — one single, generic Frame type for the whole system, not a bespoke frame per module. This turned the module/slot system from "abstract upgrades you unlock" into "real gadgets you choose to hardwire into your suit instead of carrying loose," and carried the same design principle already set for Grappling Brace: **the module grants the suit the capability to use the equipment — the equipment itself isn't what's being empowered.** That design principle still holds for every module that *does* have a standalone origin; only the "everything must eventually get one" mandate is gone.
- **Old exemption list, now just precedent, not a special case:** **Hulk Landing, Juggernaut, Hero Landing, Hell Dive, Reactive Defense Module, and Slipstream Module** were carved out early as **suit-scale kinetic events or tuned modifiers on an existing inherent mechanic**, not objects — forcing a physical-item origin onto them would have been contrived. They still cost a slot and dock via the universal Module Frame like any module (Hulk Landing/Juggernaut/Hero Landing/Hell Dive stay reachable by Exo through H.E.A.T. Sink/Overdrive Core exactly as before) — this list just isn't a notable exception anymore now that no-standalone-origin is the norm.
- **Resolved the Hero Landing fork** (see Recon's exclusive modules) before this reversal: Hero Landing is its **own equipment + the universal Frame**, not a reskin of Landing Brace's effect — a single physical item behaving in fundamentally different damage-classes per suit would have been a bigger asymmetry than the Frame system is meant to carry. Unaffected by the reversal above since it already has a standalone origin.
...
### Grappler Module — slotability TBD (reopened 2026-08-29)

**Reopened same day it was retired:** the retirement overlooked the **empty-hand-firing problem** this section's mechanism exists to solve — a docked Grappler has no held item to right-click, which is exactly why the Fire/Release/Lock keybinds and the custom client→server packet below were built in the first place. Whether the Grappler stays suit-slotable at all is **undecided again**; the mechanism below is preserved as-is (not deleted) in case slotting stays, rather than rebuilding it from scratch later if it turns out still needed.

Suit-integrated version of the **Grappler** (G.R.A.P.P.L.E.R) gadget — see `dermicraft-gadget-notes.md`. Reuses the gadget's grapple logic **identically** (Deep Rock Galactic-style direct pull, Slurry fuel charged at reload, mass-based mob interaction, normal fall-damage rules on vertical pulls, full tier upgrade ladder). The module changes only *how it is triggered and carried*, not what it does.

**Not a separate item — the universal Module Frame:** the module is the **same generic Frame type used system-wide** occupying a suit slot; the actual Grappler (with its tiers, loaded cell, and fuel grade) docks into it — this was the original model the system-wide rule was generalized from, not a bespoke Grappler-specific frame. Single source of truth — no duplicate upgrade path between handheld and docked form. Full nesting/removal details (Player → Suit → Frame → Grappler, Dock's role, specialized station dependency on the Gadget Upgrade System) are in the Grappler's own entry in `dermicraft-gadget-notes.md`.
...
- Two mutually-exclusive custom effects — *Grappler: Locked* and *Grappler: Armed* — provide a persistent HUD icon for the current state. Toggling removes one and applies the other; exactly one is present while equipped.
...
**Grappler Anchor Module is unaffected either way** — it was already its own separate universal module with its own standalone equipment origin (inventory-carry, not Frame-docked), not a version of the Grappler itself being slotted. See that entry under Legs, below.
...
- **H.E.A.T. Gun** *(brainstorm stage, 2026-08-29 — core idea down, activation mechanism still being worked out)* — Assault-exclusive **arm-mounted ranged attack module**, giving Assault a ranged option for the first time (everything else in its kit is melee/impact: Heavy Strike, One Punch, Hulk Landing, Juggernaut). **Single-target, ranged.** Weaker than One Punch — the tradeoff for reach — and leaves a **smaller crater** than One Punch's on block impact, same "leaves a mark" identity at reduced scale.
  - **Relationship to One Punch:** not a stacking pairing like Reactive Defense/Slipstream's module versions — **mutually exclusive with One Punch on the same trigger.** Equip the module once (costs an Arm slot); a **dedicated field toggle** (mirroring the Grappler's existing Lock/Armed keybind + persistent HUD status-effect pattern) then switches which one actually fires on your next empty-hand attack, without needing to re-equip anything. This is a genuinely new relationship type for this doc — the first case of a **module toggling which of two behaviors an inherent trait's own trigger produces**, rather than stacking additively alongside it (Reactive Defense Module/Slipstream Module) or being a fully separate ability (Hulk Landing, Juggernaut). Scoped to this one case for now, not a general template, same caveat the Reactive Defense/Slipstream pairing carries.
  - **Cost:** same **H.E.A.T. consumption as One Punch** — full-bank, all-or-nothing discharge on fire, not Juggernaut's continuous-partial-draw model.
  - **Reachable by Exo via H.E.A.T. Sink (confirmed 2026-08-29)** — same as Juggernaut, per the standing "modules only, not signature traits" rule; One Punch itself stays Assault-exclusive regardless. **Exo-specific wrinkle:** the field toggle described above only makes sense where there's an inherent One Punch to toggle *against* — Exo has none. On Exo, H.E.A.T. Gun instead just **directly becomes the empty-hand attack outright whenever equipped**, no toggle involved — the same shape Juggernaut already has on Exo (a granted ability with nothing native to toggle against, not a swap between two things).
  - **Open questions (why this is still brainstorm-stage):** exact damage multiplier/formula relative to One Punch. Exact crater-size ratio to One Punch's. Range value. Whether the toggle needs its own status-effect pair (à la Grappler's Locked/Armed) or can reuse simpler UI. Naming — "H.E.A.T. Gun" is a working name, not locked.
...
- **H.E.A.T. Sink Module** *(renamed to match naming convention)* — the Assault bridge. **Confirmed (2026-08-29): its equipment base is a real, required component of Assault's own construction recipe** — not just a thematic similarity, an actual shared ingredient. **On Assault:** adds more H.E.A.T. capacity — **the H.E.A.T. cap is now confirmed to exist and to double as Assault's damage-soak cap** (see `one-punch-full-breakdown.md` → H.E.A.T. — Storage & Loss for the full mechanic; exact numbers still deferred to the equipment/tier discussion). **On Exo:** *is* Exo's H.E.A.T. bank outright (Exo has no baseline H.E.A.T. generation of its own to build on) — grants access to Assault's H.E.A.T.-consuming modules (Juggernaut, H.E.A.T. Gun, and others as they're built), **not** One Punch itself (suit-signature, stays Assault-exclusive).
- **Overdrive Core Module** *(renamed to match naming convention)* — the Recon bridge, same shape mirrored for speed instead of H.E.A.T. **Confirmed (2026-08-29), matching H.E.A.T. Sink for symmetry: its equipment base is likewise a real, required component of Recon's own construction recipe.** **On Recon:** raises achievable peak speed further, directly boosting Scout Blade's damage multiplier and Slipstream's dodge chance. **On Exo:** raises Exo's speed ceiling high enough for Recon's speed-gated modules (Running Gear, Hero Landing, Hell Dive) to be meaningful once unlocked — **not** Scout Blade or Slipstream themselves (suit-signature, stay Recon-exclusive).
- **Open questions:** exact slot-count-per-tier curve for Exo (deferred to the tier/equipment pass). ~~Whether H.E.A.T. Sink's equipment base literally shares a component with Assault's construction recipe, or is merely thematically similar.~~ ~~Whether Overdrive Core gets a parallel "may double as a Recon component" property, matching H.E.A.T. Sink's, for symmetry.~~ **Both resolved 2026-08-29** — see confirmed notes above. Exact shared component/ingredient for each is still undecided, deferred to when Assault's and Recon's own construction recipes are specified.
