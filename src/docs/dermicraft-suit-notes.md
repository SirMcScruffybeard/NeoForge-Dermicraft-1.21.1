...
### Grappler Module — retired (2026-08-29)

**No longer slotable.** The Grappler (renamed from "Grapple," 2026-08-29 — see `dermicraft-gadget-notes.md`) can no longer be docked into a suit's Module Frame at all — it's handheld-only now, same as any other carried Gadget. Everything this section used to describe (the dedicated Fire/Release/Lock keybinds, the custom client→server empty-hand-fire packet, the Locked/Armed status-effect HUD indicator) existed **specifically to solve "how do you fire a suit-docked weapon with no item in hand"** — with docking gone, that whole problem (and the mechanism built to solve it) no longer applies. None of it carries over to the handheld Grappler, which already fires via ordinary right-click like every other Gadget. See the Grappler's own entry in `dermicraft-gadget-notes.md` for its unaffected standalone mechanics.

**Grappler Anchor Module is unaffected** — it was already its own separate universal module with its own standalone equipment origin (inventory-carry, not Frame-docked), not a version of the Grappler itself being slotted. See that entry under Legs, below.
