...
- **H.E.A.T. Gun** *(brainstorm stage, 2026-08-29 — core idea down, activation mechanism still being worked out)* — Assault-exclusive **arm-mounted ranged attack module**, giving Assault a ranged option for the first time (everything else in its kit is melee/impact: Heavy Strike, One Punch, Hulk Landing, Juggernaut). **Single-target, ranged.** Weaker than One Punch — the tradeoff for reach — and leaves a **smaller crater** than One Punch's on block impact, same "leaves a mark" identity at reduced scale.
  ...
  - **Open questions (why this is still brainstorm-stage):** exact damage multiplier/formula relative to One Punch. Exact crater-size ratio to One Punch's. Range value. Whether H.E.A.T. Sink lets Exo reach this the same way it reaches Juggernaut (no reason yet to think it wouldn't, per the standing "modules only, not signature traits" rule — One Punch itself would still stay Assault-exclusive either way). Whether the toggle needs its own status-effect pair (à la Grappler's Locked/Armed) or can reuse simpler UI. Naming — "H.E.A.T. Gun" is a working name, not locked.
...
- **H.E.A.T. Sink Module** *(renamed to match naming convention)* — the Assault bridge. **Confirmed (2026-08-29): its equipment base is a real, required component of Assault's own construction recipe** — not just a thematic similarity, an actual shared ingredient. **On Assault:** adds more H.E.A.T. capacity — **the H.E.A.T. cap is now confirmed to exist and to double as Assault's damage-soak cap** (see `one-punch-full-breakdown.md` → H.E.A.T. — Storage & Loss for the full mechanic; exact numbers still deferred to the equipment/tier discussion). **On Exo:** *is* Exo's H.E.A.T. bank outright (Exo has no baseline H.E.A.T. generation of its own to build on) — grants access to Assault's H.E.A.T.-consuming modules (Juggernaut, and others as they're built), **not** One Punch itself (suit-signature, stays Assault-exclusive).
