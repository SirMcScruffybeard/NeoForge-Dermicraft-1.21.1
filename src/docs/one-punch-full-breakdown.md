...
- There is no partial loss condition (e.g. no gradual decay, no percentage lost on minor events) for **passive loss** — H.E.A.T. is either fully intact or fully wiped by the conditions above. (Active *spending*, e.g. Juggernaut's continuous partial draw, is a separate, per-consumer mechanic — see that module's entry.)
...
- **H.E.A.T. Sink Module directly raises this cap** (see `dermicraft-suit-notes.md`, Exo — Cross-Suit Compatibility Modules) — confirmed to be the mechanism behind its "adds more H.E.A.T. capacity" effect on Assault.
