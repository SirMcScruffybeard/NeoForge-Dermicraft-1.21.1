...
**Hazard capability:** Follows the same **cumulative Tier hazard rule** as the rest of the mod (Floor tiers, etc.) — Tier 2 means it handles **Tier 1 safe fluids + Tier 2 Thermal Hazard (lava)** together, not a narrow "just Molten Redstone" restriction.
...
- **✅ Masticator's own Evolution Module mechanic BUILT (2026-08-23).** Full port of Cauldron's mechanic to Masticator's hazard-gated consumer shape: `installedHazardProfile()` grants whatever hazard(s) an installed Safety OR Evolution Module carries immediately (tanks read it dynamically via `VulnerableTank`'s `Supplier<HazardProfile>` constructor, replacing the old hardcoded TIER_1); an Evolution Module additionally accumulates `evolutionProgress` in ticks of completed-recipe crafting time, then runs the same particle/sound flourish (production paused via `hasCraftingInputs()` gating) and transforms the block into a real placed Charred Masticator once `evolutionThreshold` is hit, consuming the Module. Speed stays at the base 1.0x the whole time this way -- only an actual Charred Masticator gets `MachineTier.CHARRED`'s 1.25x, so a modded-in-tolerant base Masticator is deliberately slower than the real evolved thing. `CharredMasticatorBlockEntity` overrides `canEvolve()` to false, so installing any Module there (its tanks are unconditionally TIER_2 already) has no effect at all -- matches the confirmed direction that Charred Masticator itself can't evolve further under this mechanism. **Naming convention locked (2026-08-22, revised 2026-08-23):** every "ordinary machine family" hazard-gated Tier 2 evolution is named `Charred <MachineName>` -- **Charred Masticator**, **Charred Metastasizer**, **Charred Mutator**, and so on, matching the fact that these machines only ever had one-word names to begin with (unlike Cauldron→Crucible's noun-swap or Skin Tank→Chitin Tank's material-swap, both of which are identity-driven evolutions rather than hazard-gate ones). **Revised from an earlier `<Hazard> <MachineName>` ("Thermal Masticator") scheme** once the family's new charred-flesh base texture (see Charred Tumor/Hot Bone) became the actual visual identity these evolved machines will share -- naming after the texture/material now, not the hazard tag. **Open question this convention doesn't yet answer:** does "Charred" still fit once a Tier 3 for this family exists (`HazardProfile` is cumulative -- Tier 3 adds Radiation Mild on top of Thermal, not in place of it), or does Tier 3 need its own visual identity and name entirely? Not resolved; flagged for whenever a Tier 3 for this family is actually scoped. Effluentcer's and Metastasizer's mechanics are even further out -- "likely," not designed at all, but now have their name settled in advance.
...
**Reachability gap — ✅ RESOLVED (2026-08-23).** Charred Masticator exists (`TIER_2` hazard-tolerant tanks), and every Masticator-route member of the Stage 2 "Molten" fluid family now has a real recipe in datagen — see `dermicraft-crafting-notes.md`'s Molten family section for the full list and its own note on Molten Netherite (still blocked, needs a Charred Effluentcer). Most recipes use placeholder-quality item inputs/amounts (flat 110 mB/item, 30s) pending real balance numbers, per [[feedback_survival_reachability_check]] in Claude Code memory -- reachability was the immediate goal, to let the Charred Masticator evolution mechanic actually be tested end-to-end.
...
1. **Fluid-type capability** — what hazard classes the floor can conduct, following the mod's Stage-hazard ladder (see `dermicraft-project-primer.md` → Stage structure / Hazard tag hierarchy). Cumulative — each tier handles its own hazard class **and all below it**, via the tier-appropriate material (it's not "free"; higher tiers incorporate the materials that actually solve each hazard, consistent with the primer's "heat-handling ≠ radiation-handling" principle):
   ...
   - **Tier 2** — + Thermal Hazard (lava). *(Stage 1→2 gate)*
