...
**Still open:**
- The visual overlay (a creeping red/orange tint over the Cauldron's outer surface, scaled by evolution progress) -- feasible via a second render pass in the existing `DroolingCauldronBlockEntityRenderer` (same height-scaled-tinted-quad technique already used for the fluid gauge, just new geometry matching the block's outer shell rather than the fluid-pool's inset bowl geometry), deliberately deferred as a fast-follow after the core mechanic.
