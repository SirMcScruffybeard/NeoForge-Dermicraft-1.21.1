...
- Masticator's own Evolution Module is confirmed in direction and now has a real mechanism to read from (the same data map), but isn't wired up yet -- what it's upgraded from, its threshold, model. **Naming convention locked (2026-08-22):** every "ordinary machine family" hazard-gated Tier 2 evolution is named `<Hazard> <MachineName>` -- **Thermal Masticator**, **Thermal Metastasizer**, **Thermal Mutator**, and so on, matching the fact that these machines only ever had one-word names to begin with (unlike Cauldron→Crucible's noun-swap or Skin Tank→Chitin Tank's material-swap, both of which are identity-driven evolutions rather than hazard-gate ones). **Open question this convention doesn't yet answer:** `HazardProfile` is cumulative -- Tier 3 adds Radiation Mild on top of Thermal, not in place of it -- so "Thermal X" stops being accurate the moment any of these machines evolves past Tier 2. Not resolved; flagged for whenever a Tier 3 for this family is actually scoped. Effluentcer's and Metastasizer's mechanics are even further out -- "likely," not designed at all, but now have their name settled in advance.
