    ...
    private void renderModuleTab(GuiGraphics guiGraphics, int x, int y) {
        for (int i = 0; i < menu.BE.moduleSlotCount(); i++) {
            guiGraphics.blit(MODULE_SLOT_TEXTURE,
                    x + DroolingCrucibleMenu.MODULE_SLOT_X + i * DroolingCrucibleMenu.MODULE_SLOT_SPACING,
                    y + DroolingCrucibleMenu.MODULE_SLOT_Y, 0, 0, SLOT_SIZE, SLOT_SIZE, SLOT_SIZE, SLOT_SIZE);
        }
    }
