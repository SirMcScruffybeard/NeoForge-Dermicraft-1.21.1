...
public class DroolingGeodeScreen extends AbstractModScreen<DroolingGeodeMenu> {
    ...
    public DroolingGeodeScreen(DroolingGeodeMenu menu, Inventory playerInventory, Component title) {
    ...
    protected void init() {
        ...
        tabs = List.of(
                new Tab(Component.translatable("screen.dermicraft.drooling_geode.main_tab"), TAB_TEXT_COLOR),
                new Tab(Component.translatable("screen.dermicraft.drooling_geode.module_tab"), TAB_TEXT_COLOR));
    }
    ...
    protected void renderLabels(GuiGraphics guiGraphics, int pMouseX, int pMouseY) {
        ...
        if (menu.getActiveTab() == DroolingGeodeMenu.MAIN_TAB) {
    ...
    private void renderModuleTab(GuiGraphics guiGraphics, int x, int y) {
        for (int i = 0; i < menu.BE.moduleSlotCount(); i++) {
            guiGraphics.blit(MODULE_SLOT_TEXTURE,
                    x + DroolingGeodeMenu.MODULE_SLOT_X + i * DroolingGeodeMenu.MODULE_SLOT_SPACING,
                    y + DroolingGeodeMenu.MODULE_SLOT_Y, 0, 0, SLOT_SIZE, SLOT_SIZE, SLOT_SIZE, SLOT_SIZE);
        }
    }
