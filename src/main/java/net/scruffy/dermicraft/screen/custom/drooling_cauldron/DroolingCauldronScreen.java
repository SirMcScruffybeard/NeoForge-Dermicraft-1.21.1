...
public class DroolingCauldronScreen extends AbstractModScreen<DroolingCauldronMenu> {
    ...
    protected void init() {
        ...
        tabs = List.of(
                new Tab(Component.translatable("screen.dermicraft.drooling_cauldron.main_tab"), TAB_TEXT_COLOR),
                new Tab(Component.translatable("screen.dermicraft.drooling_cauldron.module_tab"), TAB_TEXT_COLOR));
    }
    ...
    private void renderModuleTab(GuiGraphics guiGraphics, int x, int y) {
        for (int i = 0; i < menu.BE.moduleSlotCount(); i++) {
            guiGraphics.blit(MODULE_SLOT_TEXTURE,
                    x + DroolingCauldronMenu.MODULE_SLOT_X + i * DroolingCauldronMenu.MODULE_SLOT_SPACING,
                    y + DroolingCauldronMenu.MODULE_SLOT_Y, 0, 0, SLOT_SIZE, SLOT_SIZE, SLOT_SIZE, SLOT_SIZE);
        }
    }
