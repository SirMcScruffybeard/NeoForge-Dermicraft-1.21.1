...
public class MetastasizerScreen extends AbstractModScreen<MetastasizerMenu> {
    ...
    // (see MasticatorScreen), same shared icons. Sits directly above the crafting-progress arrow.
    ...
    public MetastasizerScreen(MetastasizerMenu menu, Inventory playerInventory, Component title) {
    ...
    protected void init() {
        ...
        tabs = List.of(
                new Tab(Component.translatable("screen.dermicraft.metastasizer.main_tab"), TAB_TEXT_COLOR),
                new Tab(Component.translatable("screen.dermicraft.metastasizer.module_tab"), TAB_TEXT_COLOR));
    }
    ...
    protected void renderLabels(@NotNull GuiGraphics guiGraphics, int pMouseX, int pMouseY) {
        ...
        if (menu.getActiveTab() == MetastasizerMenu.MAIN_TAB) {
    ...
    /** One yellow Module slot -- nothing else on this tab, matching every other machine's own
     * bare Module-slot-only look. */
    private void renderModuleTab(GuiGraphics guiGraphics, int x, int y) {
        for (int i = 0; i < menu.BE.moduleSlotCount(); i++) {
            guiGraphics.blit(MODULE_SLOT_TEXTURE,
                    x + MetastasizerMenu.MODULE_SLOT_X + i * MetastasizerMenu.MODULE_SLOT_SPACING,
                    y + MetastasizerMenu.MODULE_SLOT_Y, 0, 0, SLOT_SIZE, SLOT_SIZE, SLOT_SIZE, SLOT_SIZE);
        }
    }
