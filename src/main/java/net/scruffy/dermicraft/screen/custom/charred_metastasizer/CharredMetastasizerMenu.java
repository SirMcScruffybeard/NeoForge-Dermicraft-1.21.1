...
/** Charred Metastasizer's menu -- identical layout/slots (Module tab included) to
 * {@code MetastasizerMenu}, just typed to {@link CharredMetastasizerBlockEntity} and checked
 * against {@link ModBlocks#CHARRED_METASTASIZER} in {@link #stillValid}. Distinct class for the
 * same reason CharredMasticatorMenu is. */
public class CharredMetastasizerMenu extends AbstractModMenu {
    ...
    public final CharredMetastasizerBlockEntity BE;
    private final Level level;

    public CharredMetastasizerMenu(int containerId, Inventory inv, FriendlyByteBuf extraData) {
        this(containerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()));
    }

    public CharredMetastasizerMenu(int containerId, Inventory inv, BlockEntity blockEntity) {
        super(ModMenuTypes.CHARRED_METASTASIZER_MENU.get(), containerId,
                MetastasizerBlockEntity.INVENTORY_SIZE + ((CharredMetastasizerBlockEntity) blockEntity).moduleSlotCount());
        this.BE = (CharredMetastasizerBlockEntity) blockEntity;
    ...
    public boolean stillValid(Player player) {
        return super.stillValid(level, player, ModBlocks.CHARRED_METASTASIZER, BE);
    }
