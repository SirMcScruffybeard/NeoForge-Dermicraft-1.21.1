...
/** Charred Render Kiln's screen -- identical GUI layout/textures to {@code RenderKilnScreen}, just
 * typed to {@link CharredRenderKilnMenu}. See that class's javadoc for why this is a separate class
 * rather than reusing RenderKilnScreen. */
public class CharredRenderKilnScreen extends AbstractModScreen<CharredRenderKilnMenu> {
    ...
    // Auto-dispense toggle -- see RenderKilnScreen's own identical constants/comment.
    ...
    public CharredRenderKilnScreen(CharredRenderKilnMenu menu, Inventory playerInventory, Component title) {
    ...
    protected void init() {
        ...
        tabs = List.of(
                new Tab(Component.translatable("screen.dermicraft.charred_render_kiln.main_tab"), TAB_TEXT_COLOR),
                new Tab(Component.translatable("screen.dermicraft.charred_render_kiln.module_tab"), TAB_TEXT_COLOR));
    }
    ...
    protected void renderLabels(@NotNull GuiGraphics guiGraphics, int pMouseX, int pMouseY) {
        ...
        if (menu.getActiveTab() == CharredRenderKilnMenu.MAIN_TAB) {
    ...
    private void renderModuleTab(GuiGraphics guiGraphics, int x, int y) {
        guiGraphics.blit(MODULE_SLOT_TEXTURE, x + CharredRenderKilnMenu.MODULE_SLOT_X, y + CharredRenderKilnMenu.MODULE_SLOT_Y, 0, 0,
