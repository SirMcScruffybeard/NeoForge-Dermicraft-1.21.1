package net.scruffy.dermicraft.screen.custom.drooling_geode;
...
public class DroolingGeodeMenu extends AbstractModMenu {
    ...
    // Module slot position -- public so DroolingGeodeScreen can draw the matching background, same
    // convention as every other machine's screen/menu coordinate split. Same coordinates as
    // DroolingCrucibleMenu's own -- no reason for these to differ between Drooling family siblings.
    ...
    public final DroolingGeodeBlockEntity BE;
    private final Level level;

    public DroolingGeodeMenu(int containerId, Inventory inv, FriendlyByteBuf extraData) {
        this(containerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()));
    }

    public DroolingGeodeMenu(int pContainerId, Inventory inv, BlockEntity blockEntity) {
        super(ModMenuTypes.DROOLING_GEODE_MENU.get(), pContainerId,
                DroolingMachineBlockEntity.INVENTORY_SIZE + ((DroolingGeodeBlockEntity) blockEntity).moduleSlotCount());
        this.BE = ((DroolingGeodeBlockEntity) blockEntity);
    ...
    public boolean stillValid(Player player) {
        return super.stillValid(level, player, ModBlocks.DROOLING_GEODE, BE);
    }
